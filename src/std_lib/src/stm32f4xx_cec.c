...
void CEC_Init(CEC_InitTypeDef* CEC_InitStruct)
{
  ...
  assert(IS_CEC_SIGNAL_FREE_TIME(CEC_InitStruct->CEC_SignalFreeTime));
  assert(IS_CEC_RX_TOLERANCE(CEC_InitStruct->CEC_RxTolerance));
  assert(IS_CEC_STOP_RECEPTION(CEC_InitStruct->CEC_StopReception));
  assert(IS_CEC_BIT_RISING_ERROR(CEC_InitStruct->CEC_BitRisingError));
  assert(IS_CEC_LONG_BIT_PERIOD_ERROR(CEC_InitStruct->CEC_LongBitPeriodError));
  assert(IS_CEC_BDR_NO_GEN_ERROR(CEC_InitStruct->CEC_BRDNoGen));
  assert(IS_CEC_SFT_OPTION(CEC_InitStruct->CEC_SFTOption));
...
void CEC_OwnAddressConfig(uint8_t CEC_OwnAddress)
{
  uint32_t tmp =0x00;
  /* Check the parameters */
  assert(IS_CEC_ADDRESS(CEC_OwnAddress));
...
FlagStatus CEC_GetFlagStatus(uint16_t CEC_FLAG)
{
  ...
  assert(IS_CEC_GET_FLAG(CEC_FLAG));
...
{
  assert(IS_CEC_CLEAR_FLAG(CEC_FLAG));
