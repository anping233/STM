...
void SPDIFRX_Init(SPDIFRX_InitTypeDef* SPDIFRX_InitStruct)
{
  ...
  assert(IS_STEREO_MODE(SPDIFRX_InitStruct->SPDIFRX_StereoMode));
  assert(IS_SPDIFRX_INPUT_SELECT(SPDIFRX_InitStruct->SPDIFRX_InputSelection));
  assert(IS_SPDIFRX_MAX_RETRIES(SPDIFRX_InitStruct->SPDIFRX_Retries));
  assert(IS_SPDIFRX_WAIT_FOR_ACTIVITY(SPDIFRX_InitStruct->SPDIFRX_WaitForActivity));
  assert(IS_SPDIFRX_CHANNEL(SPDIFRX_InitStruct->SPDIFRX_ChannelSelection));
  assert(IS_SPDIFRX_DATA_FORMAT(SPDIFRX_InitStruct->SPDIFRX_DataFormat));
...
{
  /* Check the parameters */
  assert(IS_SPDIFRX_STATE(SPDIFRX_State));
...
void SPDIFRX_ITConfig(uint32_t SPDIFRX_IT, FunctionalState NewState)
{
  /* Check the parameters */
  assert(IS_FUNCTIONAL_STATE(NewState));
  assert(IS_SPDIFRX_CONFIG_IT(SPDIFRX_IT));
...
FlagStatus SPDIFRX_GetFlagStatus(uint32_t SPDIFRX_FLAG)
{
  ...
  /* Check the parameters */
  assert(IS_SPDIFRX_FLAG(SPDIFRX_FLAG));
...
{
  /* Check the parameters */
  assert(IS_SPDIFRX_CLEAR_FLAG(SPDIFRX_FLAG));
...
{
  /* Check the parameters */
  assert(IS_SPDIFRX_CLEAR_FLAG(SPDIFRX_IT));
