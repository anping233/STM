...
void QSPI_Init(QSPI_InitTypeDef* QSPI_InitStruct)
{
  ...
  assert(IS_QSPI_SSHIFT(QSPI_InitStruct->QSPI_SShift));
  assert(IS_QSPI_PRESCALER(QSPI_InitStruct->QSPI_Prescaler));
...
void QSPI_ComConfig_Init(QSPI_ComConfig_InitTypeDef* QSPI_ComConfig_InitStruct)
{
  ...
  assert(IS_QSPI_FMODE       (QSPI_ComConfig_InitStruct->QSPI_ComConfig_FMode));
  assert(IS_QSPI_SIOOMODE    (QSPI_ComConfig_InitStruct->QSPI_ComConfig_SIOOMode));
  assert(IS_QSPI_DMODE       (QSPI_ComConfig_InitStruct->QSPI_ComConfig_DMode));
  assert(IS_QSPI_DCY         (QSPI_ComConfig_InitStruct->QSPI_ComConfig_DummyCycles));
  assert(IS_QSPI_ABSIZE      (QSPI_ComConfig_InitStruct->QSPI_ComConfig_ABSize));
  assert(IS_QSPI_ABMODE      (QSPI_ComConfig_InitStruct->QSPI_ComConfig_ABMode));
  assert(IS_QSPI_ADSIZE      (QSPI_ComConfig_InitStruct->QSPI_ComConfig_ADSize));
...
{
  /* Check the parameters */
  assert(IS_QSPI_PMM(QSPI_Match_Mode));
...
void QSPI_AutoPollingMode_SetInterval(uint32_t QSPI_Interval)
{
  ...
  /* Check the parameters */
  assert(IS_QSPI_PIR(QSPI_Interval));
...
void QSPI_MemoryMappedMode_SetTimeout(uint32_t QSPI_Timeout)
{
  ...
  /* Check the parameters */
  assert(IS_QSPI_TIMEOUT(QSPI_Timeout));
...
void QSPI_SetFIFOThreshold(uint32_t QSPI_FIFOThreshold)
{
  ...
  /* Check the parameters */
  assert(IS_QSPI_FIFOTHRESHOLD(QSPI_FIFOThreshold));
...
FlagStatus QSPI_GetFlagStatus(uint32_t QSPI_FLAG)
{
  ...
  /* Check the parameters */
  assert(IS_QSPI_GET_FLAG(QSPI_FLAG));
...
{
  /* Check the parameters */
  assert(IS_QSPI_CLEAR_FLAG(QSPI_FLAG));
...
{
  /* Check the parameters */
  assert(IS_QSPI_CLEAR_IT(QSPI_IT));
