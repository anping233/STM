...
void DSI_Init(DSI_TypeDef *DSIx,DSI_InitTypeDef* DSI_InitStruct, DSI_PLLInitTypeDef *PLLInit)
{
  ...
  assert(IS_DSI_PLL_NDIV(PLLInit->PLLNDIV));
  assert(IS_DSI_PLL_IDF(PLLInit->PLLIDF));
  assert(IS_DSI_PLL_ODF(PLLInit->PLLODF));
  assert(IS_DSI_AUTO_CLKLANE_CONTROL(DSI_InitStruct->AutomaticClockLaneControl));
  assert(IS_DSI_NUMBER_OF_LANES(DSI_InitStruct->NumberOfLanes));
...
{
  /* Check the parameters */
  assert(IS_DSI_COLOR_CODING(VidCfg->ColorCoding));
  assert(IS_DSI_VIDEO_MODE_TYPE(VidCfg->Mode));
  assert(IS_DSI_LP_COMMAND(VidCfg->LPCommandEnable));
  assert(IS_DSI_LP_HFP(VidCfg->LPHorizontalFrontPorchEnable));
  assert(IS_DSI_LP_HBP(VidCfg->LPHorizontalBackPorchEnable));
  assert(IS_DSI_LP_VACTIVE(VidCfg->LPVerticalActiveEnable));
  assert(IS_DSI_LP_VFP(VidCfg->LPVerticalFrontPorchEnable));
  assert(IS_DSI_LP_VBP(VidCfg->LPVerticalBackPorchEnable));
  assert(IS_DSI_LP_VSYNC(VidCfg->LPVerticalSyncActiveEnable));
  assert(IS_DSI_FBTAA(VidCfg->FrameBTAAcknowledgeEnable));
  assert(IS_DSI_DE_POLARITY(VidCfg->DEPolarity));
  assert(IS_DSI_VSYNC_POLARITY(VidCfg->VSPolarity));
  assert(IS_DSI_HSYNC_POLARITY(VidCfg->HSPolarity));
  /* Check the LooselyPacked variant only in 18-bit mode */
  if(VidCfg->ColorCoding == DSI_RGB666)
  {
    assert(IS_DSI_LOOSELY_PACKED(VidCfg->LooselyPacked));
  }
...
{
  /* Check the parameters */
  assert(IS_DSI_COLOR_CODING(CmdCfg->ColorCoding));
  assert(IS_DSI_TE_SOURCE(CmdCfg->TearingEffectSource));
  assert(IS_DSI_TE_POLARITY(CmdCfg->TearingEffectPolarity));
  assert(IS_DSI_AUTOMATIC_REFRESH(CmdCfg->AutomaticRefresh));
  assert(IS_DSI_VS_POLARITY(CmdCfg->VSyncPol));
  assert(IS_DSI_TE_ACK_REQUEST(CmdCfg->TEAcknowledgeRequest));
  assert(IS_DSI_DE_POLARITY(CmdCfg->DEPolarity));
  assert(IS_DSI_VSYNC_POLARITY(CmdCfg->VSPolarity));
  assert(IS_DSI_HSYNC_POLARITY(CmdCfg->HSPolarity));
...
{
  assert(IS_DSI_LP_GSW0P(LPCmd->LPGenShortWriteNoP));
  assert(IS_DSI_LP_GSW1P(LPCmd->LPGenShortWriteOneP));
  assert(IS_DSI_LP_GSW2P(LPCmd->LPGenShortWriteTwoP));
  assert(IS_DSI_LP_GSR0P(LPCmd->LPGenShortReadNoP));
  assert(IS_DSI_LP_GSR1P(LPCmd->LPGenShortReadOneP));
  assert(IS_DSI_LP_GSR2P(LPCmd->LPGenShortReadTwoP));
  assert(IS_DSI_LP_GLW(LPCmd->LPGenLongWrite));
  assert(IS_DSI_LP_DSW0P(LPCmd->LPDcsShortWriteNoP));
  assert(IS_DSI_LP_DSW1P(LPCmd->LPDcsShortWriteOneP));
  assert(IS_DSI_LP_DSR0P(LPCmd->LPDcsShortReadNoP));
  assert(IS_DSI_LP_DLW(LPCmd->LPDcsLongWrite));
  assert(IS_DSI_LP_MRDP(LPCmd->LPMaxReadPacket));
  assert(IS_DSI_ACK_REQUEST(LPCmd->AcknowledgeRequest));
...
{
  /* Check the parameters */
  assert(IS_DSI_FLOW_CONTROL(FlowControl));
...
{
  /* Check the parameters */
  assert(IS_DSI_COLOR_MODE(ColorMode));
...
{
  /* Check the parameters */
  assert(IS_DSI_SHUT_DOWN(Shutdown));
...
{
  /* Check the parameters */
  assert(IS_DSI_SHORT_WRITE_PACKET_TYPE(Mode));
                                ...
                                uint8_t* ParametersTable)
{
  ...
  /* Check the parameters */
  assert(IS_DSI_LONG_WRITE_PACKET_TYPE(Mode));
                               ...
                               uint8_t* ParametersTable)
{
  ...
  /* Check the parameters */
  assert(IS_DSI_READ_PACKET_TYPE(Mode));
...
{
  /* Check function parameters */
  assert(IS_DSI_COMMUNICATION_DELAY(CommDelay));
  assert(IS_DSI_LANE_GROUP(Lane));
...
{
  /* Check function parameters */
  assert(IS_FUNCTIONAL_STATE(State));
...
{
  /* Check function parameters */
  assert(IS_DSI_CUSTOM_LANE(CustomLane));
  assert(IS_DSI_LANE(Lane));
  assert(IS_FUNCTIONAL_STATE(State));
...
{
  /* Check function parameters */
  assert(IS_DSI_PHY_TIMING(Timing));
  assert(IS_FUNCTIONAL_STATE(State));
...
FlagStatus DSI_GetFlagStatus(DSI_TypeDef* DSIx, uint16_t DSI_FLAG)
{
  ...
  /* Check the parameters */
  assert(IS_DSI_ALL_PERIPH(DSIx));
  assert(IS_DSI_GET_FLAG(DSI_FLAG));
...
void DSI_ClearFlag(DSI_TypeDef* DSIx, uint16_t DSI_FLAG)
{
  /* Check the parameters */
  assert(IS_DSI_ALL_PERIPH(DSIx));
  assert(IS_DSI_CLEAR_FLAG(DSI_FLAG));
