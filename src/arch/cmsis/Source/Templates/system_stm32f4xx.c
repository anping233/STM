...
void SystemInit(void)
{
  ...
  /* Set HSION bit */
  RCC->CR |= (uint32_t)0x00000001;// ??HSION?  HSI oscillator ON

  /* Reset CFGR register */
  RCC->CFGR = 0x00000000;    /* System clock (SYSCLK) selected   MCO2 division by 0 MCO1 division by 0  PLLI2S clock used as I2S clock source  HSI clock selected  no clock    APB2 AHB clock not divided   APB1 AHB clock not divided   AHB system clock not divided  HSI oscillator used as the system clock  HSI oscillator selected as system clock*/

  /* Reset HSEON, CSSON and PLLON bits */
  RCC->CR &= (uint32_t)0xFEF6FFFF; /* PLL off  Clock security system OFF   HSE oscillator OFF */

  /* Reset PLLCFGR register */
  RCC->PLLCFGR = 0x24003010; /* PLLQ = 4  HSI clock selected as PLL and PLLI2S clock entry  PLLP = 2  PLLN = 192  PLLM = 16*/
  /* Reset HSEBYP bit */
  RCC->CR &= (uint32_t)0xFFFBFFFF;/* HSE oscillator not bypassed */
